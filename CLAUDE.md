...
pg_cocoon is a PostgreSQL extension written in Rust (using pgrx 0.17) that provides time-series data management on top of native PostgreSQL declarative partitioning. It supports PostgreSQL 14–18 (default: 17).
...
- **`lib.rs`** — Extension entry point. Defines `_PG_init()`, catalog table schemas (`cocoon_hypertable`, `cocoon_partition`) via `extension_sql!`, and the `pg_cocoon.mock_now` GUC for testing.
- **`partition.rs`** — Core partitioning logic. Contains `cocoon_create_table()` (converts an empty table to a partitioned hypertable), `cocoon_partition_info()`, `cocoon_hypertable_info()`, and helpers for interval math, partition naming, and initial partition creation.
...
Python-based (pytest + psycopg). Fixtures in `conftest.py` manage Docker container lifecycle and per-test database creation. The `pg_cocoon.mock_now` GUC allows deterministic time-based testing. Test files: `test_partitioning.py`, `test_functions.py`, `test_worker.py`.
...
- The background worker requires `shared_preload_libraries=pg_cocoon` and skips execution on replicas.
