...
(Requires PG_COCOON_IMAGE env var)
"""
...
def setup_bench_table(conn, table_name="bench_metrics", n_devices=1000, n_points=10000):
    ...
    conn.execute("SET pg_cocoon.mock_now = '2025-01-15 12:00:00+00'")
    conn.execute(f"""
...
class TestBenchCompression:
    ...
    def bench_db(self, db):
        ...
        db.execute(
            "SELECT cocoon_enable_compression('bench_metrics', "
    ...
    def _get_partition(self, conn, ts="2025-01-15"):
        ...
        parts = conn.execute(
            "SELECT partition_name FROM cocoon_partition_info('bench_metrics') "
    ...
    def test_compression_ratio(self, bench_db):
        ...
        stats = bench_db.execute(
            ...
            f"FROM cocoon_compression_stats('bench_metrics') "
    ...
    def test_compression_throughput(self, bench_db):
        ...
        bench_db.execute(f"SELECT cocoon_compress_partition('{part_name}')")
        bench_db.commit()
        compress_time = time.monotonic() - start

        # Decompress
        start = time.monotonic()
        bench_db.execute(f"SELECT cocoon_decompress_partition('{part_name}')")
        bench_db.commit()
